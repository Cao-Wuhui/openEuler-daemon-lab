...
这是因为当前实验平台是 Docker 容器，PID 1 不是 systemd。因此，本实验不能使用 systemctl 管理 rsyslog，而应采用手动启动 rsyslogd 的方式。
...
脱离控制终端
被 PID 1 接管
结合代码中 fork()、setsid()、再次 fork()、关闭文件描述符、切换工作目录等操作
需要特别说明的是：
S 仅表示当前运行状态是睡眠状态
S 不是判断守护进程的唯一依据
